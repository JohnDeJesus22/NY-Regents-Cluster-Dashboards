...
alg_two = func.load_postgres('Alg2CC')

# convert dates to string data type
alg_two['Regents Date'] = alg_two['Regents Date'].astype('str')
...
app.layout=html.Div(children=[
                # main title
                html.H1(children='Algebra 2 CC Regents Cluster Analysis Dashboard',
                        style={'textAlign':'center','fontFamily': 'Arial'}),
                      
                # subtitle description
                html.H3(children=dcc.Markdown(alg_two_gen_description),
                ...
                #info for links
                html.Div(dcc.Markdown(alg_two_additional_info),style={'fontFamily': 'Arial'})
...
def update_double_bar(selected_exam):
    return func.nested_bar(alg_two, selected_exam)
...
def excluded_clusters_double(exam_date):
    return func.reveal_missing_clusters(alg_two, exam_date)
...
def update_simple_bar(selected_exam):
    return func.percentage_bar(alg_two, selected_exam)
...
def update_cluster_timeseries(cluster_list):
    return func.cluster_time_series(alg_two, cluster_list)
...
def time_series_click_bar(clickData,cluster_list):
    return func.time_series_bar(alg_two, clickData, cluster_list)
