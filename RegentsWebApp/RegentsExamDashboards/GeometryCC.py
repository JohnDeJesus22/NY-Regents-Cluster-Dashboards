...
def excluded_clusters_double(exam_date):
    if exam_date !='All Exams':
        ...
        if len(geo.Cluster.unique())==len(filtered_by_date.Cluster.unique()):
            return 'All clusters assessed in {}.'.format(exam_date)
        
        else:
            missing_clusters=[]
            for i in geo.Cluster.unique().tolist():
...
def time_series_click_bar(clickData,cluster_list):
    ...
    if clickData==None:
        ...
        filtered=geo[(geo['Regents Date']==exam_date) & (geo.Cluster.isin(cluster_list))]
