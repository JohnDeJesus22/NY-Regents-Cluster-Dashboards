#algebra 1 Dashboard #6/20/18 still need to create cluster title column
...
alg_one=pd.read_csv('PreppedAlg1QuestionBreakdown.csv',encoding='latin1')#,usecols=['ClusterTitle',
                                        #'Cluster', 'DateFixed','Regents Date', 'Type'])

'''
#below done for improving display of dropdown options for bar charts
alg_one['Regents Date']=pd.to_datetime(alg_one['Regents Date'])#,format='%m/%d/%Y')
...
alg_one['DateFixed']=alg_one['Regents Date'].apply(month_year)


alg_one.to_csv('PreppedAlg1QuestionBreakdown.csv',index=False)
'''
...
def update_double_bar(selected_exam):
    ...
    type_group=alg_one.groupby(['Type', 'Cluster', 'DateFixed']).size().reset_index(name='counts')
...
def excluded_clusters_double(exam_date):
    if exam_date !='All Exams':
        filtered_by_date=alg_one[alg_one.DateFixed==exam_date]
        
        if len(alg_one.Cluster.unique())==len(filtered_by_date.Cluster.unique()):
            return 'All clusters assessed in {}.'.format(exam_date)
        
        else:
            missing_clusters=[]
            for i in alg_one.Cluster.unique().tolist():
...
def update_simple_bar(selected_exam):
    ...
    if selected_exam !='All Exams':
        ...
        date_group=alg_one.groupby(['DateFixed','Cluster']).size().reset_index(name='count')
        ...
        #get freqency
        alg_one['freq']=alg_one.groupby('Cluster')['Cluster'].transform('count')
        
        #drop duplicates
        new_alg_one=alg_one.drop_duplicates(subset=['Cluster'],keep='first')
        
        #percentage of exam column
        new_alg_one['freq_pct']=new_alg_one['freq'].apply(lambda x: x/sum(new_alg_one['freq']))
        
        #create trace
        new_trace=[{'x': new_alg_one['Cluster'],
                    'y':new_alg_one['freq_pct'],
                    'type':'bar'}]
        return {'data': new_trace,
                'layout':{
                        'plot_bgcolor':'#EAEAD2',
...
def update_cluster_timeSeries(cluster_list):
    ...
    for cluster in cluster_list:
        ...
        sel_cluster=alg_one[alg_one['Cluster']==cluster]
...
def time_series_click_bar(clickData,cluster_list):
    ...
    if clickData==None:
        filtered=alg_one[alg_one.Cluster.isin(cluster_list)]
        ...
        filtered=alg_one[(alg_one['Regents Date']==exam_date) & (alg_one.Cluster.isin(cluster_list))]
